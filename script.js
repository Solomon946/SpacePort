...
(function initModals() {
  ...
  const projects = [
    {
      ...
      desc: 'A comprehensive real-time analytics platform built for enterprise teams. Features AI-powered anomaly detection, customizable drag-and-drop dashboards with 40+ widget types, and a multi-tenant architecture supporting 10,000+ concurrent users with sub-200ms query responses.',
      longDesc: 'Built with a micro-frontend architecture, each dashboard widget is independently deployable. The backend uses TimescaleDB for time-series data and Redis for caching. Integrated with 50+ data sources via a custom connector framework.',
      tech: ['Next.js', 'TypeScript', 'D3.js', 'PostgreSQL', 'TimescaleDB', 'Redis', 'Docker', 'AWS ECS'],
    },
    {
      title: 'Orbital Design System',
      hue: 160,
      tag: 'Design System',
      desc: 'A comprehensive design system and component library serving 8 product teams. Features 200+ accessible components, automated visual regression testing, Figma token sync, and a fully-searchable documentation site with live code playground.',
      longDesc: 'Built on Radix UI primitives with a custom theming layer. Implemented automated Chromatic visual testing, semantic versioning, and a changelog bot. Reduced cross-team design inconsistencies by 78% in the first quarter post-launch.',
      tech: ['React', 'TypeScript', 'Storybook', 'Figma', 'Jest', 'Chromatic', 'Rollup', 'Radix UI'],
    },
    {
      title: 'Pulse E-Commerce',
      hue: 30,
      tag: 'E-Commerce Platform',
      desc: 'High-performance e-commerce platform achieving sub-100ms TTI through aggressive code splitting and edge caching. Features 3D product configurators built with Three.js, a ML-powered recommendation engine, and one-click checkout with 15+ payment methods.',
      longDesc: 'Implemented ISR with on-demand revalidation for product pages. The 3D configurator supports real-time material swapping and AR preview on iOS. The recommendation engine uses collaborative filtering with a 34% increase in average order value.',
      tech: ['Next.js', 'Three.js', 'Stripe', 'Redis', 'PostgreSQL', 'TensorFlow.js', 'Vercel', 'Cloudflare'],
    },
    {
      title: 'Synth AI Chat',
      hue: 280,
      tag: 'AI Application',
      desc: 'AI-native conversational interface with real-time streaming responses, persistent conversation memory, multi-modal input including voice and images, and the ability to execute code in a sandboxed environment. Supports 20+ AI model providers.',
      longDesc: 'Built a custom streaming protocol over WebSockets for ultra-low latency responses. The memory system uses vector embeddings with pgvector for semantic search across conversation history. Voice input uses Whisper API with custom noise cancellation.',
      tech: ['React', 'FastAPI', 'OpenAI', 'WebSocket', 'pgvector', 'Whisper', 'Docker', 'Kubernetes'],
    },
  ];
  ...
  function openModal(index) {
    ...
    content.innerHTML = `
      <div class="modal-project-img" style="background: linear-gradient(135deg, hsl(${p.hue},30%,12%), hsl(${p.hue + 30},40%,18%))">
        <span style="font-family:var(--font-mono);font-size:2rem;color:hsla(${p.hue},60%,70%,0.25)">{&nbsp;}</span>
      </div>
